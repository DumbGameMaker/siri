...
exports.newMsg = async (message) => {
    ...
    if (!message.content.toLowerCase().includes("say")) {
      fs.readFile("./chatCmds.json", (err, unparsedChatCmds) => {
        ...
        var chatCmdsEmbed = new Discord.MessageEmbed()
          ...
          .setDescription(
            "The following is a list of my chat commands. Tip: Be sure to use one of my prefixes before any of these commands."
          )
      ...
      if (message.content.toLowerCase().includes("greet")) {
        var targetGreet = message.content.substr(
          functions.prefix_length + 7,
      ...
      if (message.content.toLowerCase().includes("request:")) {
        client.users.fetch(789658327348936744n).then((user) => {
          user.send(
            message.author.username +
              " has requested for you to add the following feature to me: " +
              message.content.substr(
                11 + functions.prefix_length,
                message.content.length
              )
          );
        });
        ...
        message.channel.send(
          "Ok " +
            message.author.username +
            ", I told Ozarks about the new feature you requested! :smiley:"
        );
      }
      ...
      if (message.content.toLowerCase().includes("user info")) {
        ...
        if (user) {
          ...
          message.channel.send(
            "Hey, you need to tag someone for me to show you info about them, sorry."
          );
        }
      }
      ...
      if (message.content.toLowerCase().includes("get support from ozarks")) {
        ...
        client.users.fetch(789658327348936744n).then((ozarks) => {
          ozarks.send(
            message.author.username +
              " asked me to invite you to " +
              message.guild.name +
              " because he/she needs support with me from you. " +
              invite.url
          );
        });

        message.channel.send(
          "If i have permission to create an invite, then I have sent Ozarks the invite and requested support from him. He should join this server and assist you soon."
        );
      }

      if (message.content.toLowerCase().includes("import")) {
        var emoji = message.content.substr(
          functions.prefix_length + 8,
          message.content.length
        );
        ...
        if (taggedUser) {
          if (taggedUser.bot) {
            message.channel.send(
              "Here is " +
                taggedUser.username +
                "'s invite link: https://discord.com/api/oauth2/authorize?client_id=" +
                taggedUser.id +
      ...
      if (message.content.toLowerCase().includes("hey bixby tweet")) {
        if (message.author.id == 789658327348936744n) {
          ...
          var twitter_application_secret =
            "vXWnBZWIaf1XctQw6gREHXrncEApgLwwdaI1rPXB3kuMwYPS1x"; // API Secret
          var twitter_user_access_token =
            "1339293659318128644-g3u2jS2CY3NxNBpysGaDjDUYHf7WHR"; // Access Token
          var twitter_user_secret =
            "wktIGdDhVgSWe0kHXnhliHgFZNTIjYvNKsIZNQ50DxwFG"; // Access Token Secret
      ...
      if (
        message.content.toLowerCase().includes("what can you do") ||
        message.content.toLowerCase().includes("all commands")
      ) {
        const commandsEmbed = new Discord.MessageEmbed()
          ...
          .setDescription(
            "The following is a list of the different pages of commands I have."
          )
          .setThumbnail(client.user.avatarURL())
          .addField(
            "Automatic Actions",
            "These are things I do automatically for your convenience."
          )
          .addField(
            "Moderation Commands",
            "These are commands I have related to moderation that you need permission to use."
          )
          .addField(
            "Utility Commands",
            "These are commands perform an action, but you don't need permission to use them."
          )
          .addField(
            "Chat Commands",
            "These are commands that are purely chat related and fun."
          )
      ...
      if (message.content.toLowerCase().includes("utility commands")) {
        const utilCmdsEmbed = new Discord.MessageEmbed()
          ...
          .setTitle("Commands List")
          .setAuthor(
            "Bixby Utility Commands",
            client.user.avatarURL(),
            "https://discord.js.org"
          )
          .setDescription(
            "The following is a list of my utility commands. Tip: Be sure to use one of my prefixes before any of these commands."
          )
          .setThumbnail(client.user.avatarURL())
          .addField("Ping", "This command replies with Pong", true)
          .addField(
            "Greet `user`",
            "This command greets the user you say.",
            true
          )
          .addField(
            "Uptime",
            "This command replies with my uptime in a readable format.",
            true
          )
          .addField(
            "Request: `new-feature`",
            "This command sends Ozarks the new feature you are requesting.",
            true
          )
          .addField(
            "User Info",
            "This command replies with info about the tagged user.",
            true
          )
          .addField(
            "Guild Info",
            "This command replies with info about the server.",
            true
          )
          .addField("Expand `emoji`", "Expands the custom emoji you mention.")
          .addField(
            "Import `emoji`",
            "Imports/adds to this server the foreign emoji you mention."
          )
          .addField(
            "Bot Invite `@bot`",
            "Sends the invite for the bot you mention"
          )
          .addField(
            "Get support from Ozarks",
            "Sends an invite to Ozarks, my developer so he can join this server and provide support with me."
          )
          .addField(
            "Version __or__ Changelog",
            "Gives a list of new features on me."
          )
          .addField("Crash", "Attempts to crash me.")
          .addField("Tweet", "Attempts to make me tweet.")
          .addField(
            "What can you do __or__ All commands",
            "Shows the different lists of commands I have."
          )
          .setImage(client.user.avatarURL())
          .setTimestamp()
          .setFooter(message.author.username, message.author.avatarURL());
      ...
      if (
        ...
        if (
          !(
            functions.detectSwears(message) == true ||
          ...
          message.channel.send(
            "Whoa, hold up " +
              message.author.username +
              ". You are not going to make me say a bad word."
          );
        } else if (
          message.content.includes("@everyone") ||
          message.content.includes("@here")
          ...
          message.channel.send(
            "Whoa, hold up " +
              message.author.username +
              ". You are not going to make me ping everyone."
          );
        }
      } else {
  ...
  if (message.content.startsWith("@override")) {
    if (
      message.author.id == 789658327348936744n ||
      message.author.id == 272876963100753922n ||
      message.author.id == 749339332711546992n
    ) {
      try {
      ...
      message.channel.send(
        "I don't know how you found out about this command, but unfortunately only my developer, Ozarks, has the permission to use it."
      );
    }

    if (
      (message.content.toLowerCase().includes("define eee") &&
        message.content.includes("Eee")) ||
      (message.content.toLowerCase().includes("what is eee") &&
        message.content.includes("Eee"))
    ) {
      message.channel.send(
        "E is equal to 2e. E is also equal to the square root of 16. So E = 4, e = 2, and Eee = (4×2×2) = 16."
      );
    }
    ...
    if (message.content.toLowerCase() === "hi bixby") {
      if (message.author == 794794367377735710n) {
        ...
        message.channel.send(
          "Hello. :eyes: I'm right here to help you, " +
            message.author.username +
            " :smiley:"
        );
      }
