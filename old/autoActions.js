...
exports.newMsg = (message) => {
  ...
  if (message.guild) {
    if (message.channel.name.toLowerCase().includes("count")) {
      const countUser = message.guild.members.cache.find((member) =>
        member.user.username.toLowerCase().includes("count")
      );
      ...
      if (
        countBot == false ||
        message.channel.name.toLowerCase().includes("bixby")
      ) {
        if (Number(message.content) + 1) {
          message.channel.messages.fetch({ limit: 2 }).then((messages) => {
            ...
            if (lastMessage) {
              if (
                Number(message.content) - Number(lastMessage.content) == "1" &&
  ...
  if (
    ...
    (message.content.toLowerCase().includes("bixby") ||
      message.content.includes("<@" + client.user.id + ">"))
  ) {
    try {
      message.react("<:sus:826251069780525056>");
    } catch (error) {}
  }
  ...
  if (message.content.toLowerCase().includes("ozark")) {
    if (Math.floor(Math.random() * 1) == 0) {
      try {
        message.react("<:ozarks:815662772577894490>");
      } catch (error) {}
    } else {
      try {
        message.react("<a:PetOzarks:796211034876411914>");
      } catch (error) {}
    }
  }
  ...
  if (message.content.toLowerCase().includes("daniel")) {
    if (Math.floor(Math.random() * 1) == 0) {
      try {
        message.react("<:daniel:822105537709604956>");
      } catch (error) {}
    } else {
      try {
        message.react("<a:PetDaniel:796210478359379978>");
      } catch (error) {}
    }
  }
  ...
  if (functions.startsWithPrefix) {
    if (message.content.toLowerCase().includes("automatic actions")) {
      const autoActionsEmbed = new Discord.MessageEmbed()
        ...
        .setDescription(
          "The following is a list of the different actions I perform on my own."
        )
        .setThumbnail(client.user.avatarURL())
        .addField(
          "Welcome and leave messages",
          "If a channel name contains the word __portal__ in it, I automatically send welcome and leave messages when users join and leave the server. I also give people the Member role and bots the Bot role automatically when joining."
        )
        .addField(
          "Censors swear words",
          "When members use swear words in their messages, I automatically send a warning message to discourage it."
        )
        .addField(
          "Controls counting channel",
          "If a channel name contains the word count ot bixby count, I automatically control that channel to make sure bad numbers and words are kept out of it. I make sure only numbers in order are in that channel."
        )
...
exports.newMember = (member) => {
  ...
  const welcomeChannel = member.guild.channels.cache.find((channel) =>
    channel.name.toLowerCase().includes("portal")
  );
  ...
  if (member.user.bot) {
    var botRole = member.guild.roles.cache.find((role) =>
    ...
    var memberRole = member.guild.roles.cache.find((role) =>
